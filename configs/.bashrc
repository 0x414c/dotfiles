...
## RETURN CODES:
##   `2' if no args provided;
##   `1' if one or more files is not readable;
##   or return code of the last executed command.
function __source__ {
    [[ "${#}" -lt 1 ]] && return 2
    ...
    for __file__ in "${@}"; do
        if [[ -r "${__file__}" ]]; then
            ...
            [[ ! "${__retcode__}" -eq 1 ]] && __retcode__="${?}"
        else
            command echo "${FUNCNAME[0]}: file \`${__file__}' does not exist or is not readable" 1>&2
...
function __init_env_vars__ {
    ...
    ## Backup suffix for `cp':
    export SIMPLE_BACKUP_SUFFIX='-'

    ## Version control method for `cp' backups:
...
function __init_shell_opts__ {
    ...
    HISTTIMEFORMAT='%a, %d-%b-%y %l:%M:%S %p  '
    IFS=$' \n\t'
...
function __init_aliases__ {
    ...
    func_exists 'manc' && alias man='manc'
    ...
    alias lr='ls -AR | grep ":$" | sed -e '\''s/:$//'\'' -e '\''s/[^-][^\/]*\//--/g'\'' -e '\''s/^/   /'\'' -e '\''s/-/|/'\'' | less'
...
function __init_prompts__ {
    ...
    ##   \D{FMT}   Date formatted according to `FMT' string (using `strftime')
    ...
    local __date__='\D{%k:%M:%S}'
    local __user__='~\u'
    local __host__='@\H'
    local __name__=':\w'
    local __stat__='$([[ "${?}" == 0 ]] && echo -n '✔' || echo -n '✘')'
    local __jobs__='\!:\#:\j:'"${__stat__}"
    func_exists '__git_ps1' && local __git__="\$(__git_ps1 '  ${__section_start__}  git:%s')" || local __git__=''
    func_exists '__venv_ps1' && local __venv__="\$(__venv_ps1 '  ${__section_start__}  venv:%s')" || local __venv__=''
    local __path__='\W'"${__git__}${__venv__}"
    local __mark__='\$'

    ## Build prompts:
    ## NOTE: Exporting PS{1, 2, 3, 4} can break other shells (e.g. Dash) if they are started from Bash session.
